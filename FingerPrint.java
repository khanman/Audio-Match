...
public class FingerPrint {
	...
	public ArrayList<Double> scan() {
		...
		for (int i = 0; (i + chunkSize < singleChannelIntData.length); i +=
				chunkSize) {
			LOGGER.info("Processing chunk : "+(i*chunkSize));
			ComplexNumber[] complexFile =
					createComplexNumberData(
							Arrays.copyOfRange(singleChannelIntData, i, i
									+ chunkSize), chunkSize);
	...
	private double getMaxOfArray(ComplexNumber[] range){
		double max = range[0].magnitude();
		for(ComplexNumber c: range){
			if(max > c.magnitude()){
				max = c.magnitude();
			}
		}
	...
	/**
	 * Splits the left channel data into chunks of size power 2 closest to
	 * the sampling frequency i.e. 32768 samples
	 * 
	 * @param leftChannelData
	 * @return chunks of 32768 samples
	 */
	/*private ArrayList<float[]> createChunks(float[] leftChannelData) {
		int chunkSize = Util.getNextPower(4096, 2);
		ArrayList<float[]> chunksList = new ArrayList<float[]>();

		int numberOfChunks = leftChannelData.length / chunkSize;
		int from;
		int to;
		for (int i = 0; i < numberOfChunks; i++) {
			from = chunkSize * i;
			to = chunkSize * (i + 1);
			chunksList.add(Arrays.copyOfRange(leftChannelData, from, to));
		}
		return chunksList;
	}*/
	...
	/**
	 * Converts the audio input stream to a down sampled single channel
	 * format
	 * @return
	 */
	/*private AudioInputStream downSample() {
		AudioInputStream convertedAIS = null;
		AudioInputStream lowResAIS = null;
		ByteArrayOutputStream lowResOS = new ByteArrayOutputStream();
		// Checking if conversion is possible
		System.out.println("Checking if conversion to DOWN_SAMPLED WAVE is possible...");
		if(AudioSystem.isConversionSupported(Util.REDUCED_WAV_FORMAT, audioInputStream.getFormat())) {
			try {
				System.out.println("Before convo: "+audioInputStream.available());			
				System.out.println("Conversion possible!");
				convertedAIS = AudioSystem.getAudioInputStream(Util.REDUCED_WAV_FORMAT, audioInputStream);
//				AudioSystem.write(convertedAIS, AudioFileFormat.Type.WAVE, lowResOS);
//				byte[] tempMem = lowResOS.toByteArray();
//				System.out.println("temp mem size: "+tempMem.length);
				byte[] originalAudioData = retrieveByteData(audioInputStream);
				ByteArrayInputStream lowResIS = new ByteArrayInputStream(originalAudioData);
				lowResAIS = new AudioInputStream(lowResIS, Util.REDUCED_WAV_FORMAT, originalAudioData.length / Util.REDUCED_WAV_FORMAT.getFrameSize());
				System.out.println("After convo: "+lowResAIS.available());
			} catch (IOException e) {
				System.out.println("IO Exception in downsample method: "+e.getMessage());
			}
			return lowResAIS;
		} 
		return lowResAIS;
